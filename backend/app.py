...
# ── CORS FIX ─────────────────────────────────────────────────
# CRITICAL: origins="*" + supports_credentials=True is INVALID per CORS spec
# Browsers reject it and you get "Failed to fetch" on every request.
# Fix: set supports_credentials=False (JWT goes in Authorization header, not cookie)
CORS(app,
     origins="*",
...
def add_cors_and_json(response):
    """Ensure CORS headers on every response + force JSON on API routes."""
    # Always set CORS headers (flask-cors may miss some edge cases)
    response.headers["Access-Control-Allow-Origin"]  = "*"
    response.headers["Access-Control-Allow-Headers"] = "Content-Type, Authorization, Accept"
    response.headers["Access-Control-Allow-Methods"] = "GET, POST, PUT, DELETE, OPTIONS"
    response.headers["Access-Control-Max-Age"]       = "3600"
    # Force JSON content-type on all /api/ routes (except the file stream endpoint)
